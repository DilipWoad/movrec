...
const SimilarMovieContainer = ({ movieId }) => {
  ...
  const similarMovies = async () => {
    const res = await fetch(
      MOVIE_INFO_URL +
        movieId +
        `/recommendations${viewMore ? "?language=en-US&page=2" : ""}`,
      options
    );
    ...
    <div className="bg-black text-white p-2 rounded-xl">
            ...
            (movie) =>
              movie?.poster_path && (
                <MovieCard
                  ...
                  css={
                    "min-w-32 flex md:p-2 rounded-lg md:min-w-60 md:h-auto md:flex md:justify-center hover:cursor-default "
                  }
                  postercss={"min-w-32 md:h-56 rounded-md hover:cursor-pointer"}
                />
              )
          )}
      </div>
      <div className="flex gap-4 mt-5 text-lg justify-center">
        <div
          ...
          className={`${
            viewMore
              ? "text-gray-500 hover:cursor-pointer hover:border-b-[1px] hover:border-white"
