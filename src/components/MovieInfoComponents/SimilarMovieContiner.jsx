    ...
    return(
        <div className="bg-black text-white p-2 rounded-xl">
            ...
            <div className="flex flex-wrap justify-center mt-5 gap-3">
                {similar && similar.map((movie)=>(movie?.poster_path &&<MovieCard key={movie.id} movieId={movie.id} poster={movie?.poster_path} css={"min-w-32 flex md:p-2 rounded-lg md:min-w-60 md:h-auto md:flex md:justify-center hover:cursor-default "} postercss={"min-w-32 md:h-56 rounded-md hover:cursor-pointer"}/>) ) }
            </div>
