...
const GeminiSearchBar = () => {
  ...
  const handleSearchClick = async () => {
    ...
    //useHook here
    try {
      const { geminiResults, geminiMovie } = await useGeminiSearch(
        searchPrompt
      );
      dispatch(
        addGeminiMovies({
          movieNames: geminiMovie,
          movieResults: geminiResults,
        })
      );
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="pt-[50%] md:pt-[10%] flex justify-center">
      <form
        className="w-[90%] bg-black md:w-1/2 grid grid-cols-12 rounded-md opacity-90"
        onSubmit={(e) => e.preventDefault()}
      >
        <input
          ref={prompt}
          className="col-span-9 p-2 m-2 placeholder:text-gray-400"
          type="text"
          placeholder={lang[language].geminiPlaceholder}
        />
        <button
          onClick={handleSearchClick}
          className="bg-red-700 col-span-3 m-2 rounded-sm text-white font-bold"
        >
